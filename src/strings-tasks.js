...
function countVowels(str) {
  ...
  for (let i = 0; i < vowels.length; i += 1) {
    const matches = [];
    let index = str.indexOf(vowels[i]);

    while (index !== -1) {
      matches.push(index);
      index = str.indexOf(vowels[i], index + 1);
    }
...
function isPalindrome(str) {
  const strTrimed = str.toLowerCase().replace(/[ ,.!?]/g, '');
  const strReversed = strTrimed.split('').reverse().join('');
  return strTrimed === strReversed;
}
...
function findLongestWord(sentence) {
  ...
  const lengthOfWordsSorted = [...lengthOfWords].sort((a, b) => b - a);
  const index = lengthOfWords.indexOf(lengthOfWordsSorted[0]);
  return words[index];
}
...
function invertCase(str) {
  ...
  const charsInverted = chars.map((char) => {
    return char === char.toUpperCase()
      ? char.toLowerCase()
      : char.toUpperCase();
  });
  return charsInverted.join('');
}
